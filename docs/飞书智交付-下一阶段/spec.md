...
最近核验：2026-05-06（Asia/Shanghai）

当前基线：基于工作区现有未提交代码与文档核验；已补齐“会话确认需求 -> 结构化需求文档 -> 飞书发送 -> 自动启动 Pipeline -> 前端展示绑定 Pipeline”链路。

## 0. 2026-05-06 新基线

本轮围绕赛题主链路补齐需求入口到 Pipeline 的自动衔接：

1. 会话对话仍承担自然语言需求理解、追问歧义、整理上下文的入口职责。
2. 当用户在对话中表达“发布需求 / 需求确认完成 / 进入流水线 / 开始交付”等确认意图时，后端发布流程会异步执行结构化需求归档。
3. 发布流程会基于 Agent 输出的 `Requirement` 和任务拆解生成飞书结构化需求文档，并通过飞书 bot 发送给会话归属用户。
4. 发布流程会自动创建来源为该 Session 的 PipelineRun，并立即调用 `StartPipelineRun` 进入队列执行，不再停留在 draft。
5. 会话页会轮询并展示当前会话绑定的 PipelineRun 状态、当前阶段和入口按钮，用户可以从对话页直接进入 Pipeline 工作台。
